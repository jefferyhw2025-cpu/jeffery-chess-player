...
function saveFriendMode() {
  try {
    ...
    // Friend records stay active for this session if storage is unavailable.
  }
}
...
function saveAchievements() {
  try {
    ...
    // Achievements remain available for this session.
  }
}
...
function saveMasterNoHintWinStreak() {
  try {
    ...
    // The streak remains available for this session.
  }
}
...
function saveCurrentAccountId() {
  try {
    ...
    // The account switch still works for this session if storage is unavailable.
  }
}
...
function markInitialOnlineRankMigrated() {
  try {
    ...
    // Migration is best-effort; local rank remains protected either way.
  }
}
...
function skipInitialOnlineRankMigration() {
  try {
    ...
    // The prompt may appear again if storage is unavailable.
  }
}
...
function saveOfficialRankProfile() {
  try {
    ...
    // Official rank cache is optional.
  }
}
...
function saveProfileActivity() {
  try {
    ...
    // Profile stats stay available for this session if storage is unavailable.
  }
}
...
function saveVolumeSettings() {
  try {
    ...
    // Audio settings stay active for this session if storage is unavailable.
  }
}
...
function saveRankedMode() {
  try {
    ...
    // Ranked mode remains available for this session if storage is unavailable.
  }
}
...
function saveProfessionalLeagueMode() {
  try {
    ...
    // League mode remains available for this session if storage is unavailable.
  }
}
...
function saveDailyProgress() {
  try {
    ...
    // Daily tasks remain visible for this session if storage is unavailable.
  }
}
...
function saveDailyStars() {
  try {
    ...
    // Daily stars remain visible for this session if storage is unavailable.
  }
}
...
function saveDailyStreak() {
  try {
    ...
    // Daily streak stays visible for this session if storage is unavailable.
  }
}
...
function saveTutorialLessons() {
  try {
    ...
    // Tutorial progress remains visible for this session if storage is unavailable.
  }
}
...
function markLeagueRewardClaimed(seasonId, achievementId) {
  try {
    ...
    // Reward badges still unlock through the ordinary achievement storage.
  }
}
...
function saveCurrentGame() {
  try {
    ...
    // The active game still works for this session if storage is unavailable.
  }
}
...
function saveContinuePromptDismissed() {
  try {
    ...
    // Continue prompt can reappear if storage is unavailable.
  }
}
...
function createProfileBackupSnapshot(reason = "manual") {
  ...
  for (const key of trackedProfileStorageKeys()) {
    try {
      ...
      // Skip unreadable keys; the rest of the backup is still useful.
    }
  }
  return {
    version: appVersion,
...
function maxStoredAchievementCount(data) {
  ...
  for (const [key, value] of Object.entries(data)) {
    ...
    try {
      ...
      // Ignore unreadable achievement entries in the preview.
    }
  }
  return count;
}
...
async function applyAvailableUpdate() {
  ...
  try {
    ...
    // Cache clearing is a best-effort step before the reload.
  }
...
async function discoverLanServerBase() {
  for (const base of localAdminServerBases) {
    try {
      ...
      // Try the next likely local LAN server address.
    }
  }
  return "";
}
...
function uniqueLanInfoBases() {
  ...
  bases.push(...localAdminServerBases);
...
async function fetchLanInfoStatus() {
  for (const base of uniqueLanInfoBases()) {
    try {
      ...
      // Try the next likely server address.
    }
  }
...
function loadLanFen(fen, last = null) {
  ...
  try {
    ...
    // Keep the local board if the remote state is unavailable.
  }
}
...
function registerOfflineApp() {
  ...
  navigator.serviceWorker.register("./service-worker.js").catch(() => {
    // Offline mode is a bonus; gameplay should continue even if registration fails.
  });
}
